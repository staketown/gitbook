...
```bash
bash <(curl -s https://raw.githubusercontent.com/staketown/cosmos/master/nolus/install.sh)
```
...
git checkout v0.3.0
make install
nolusd version # v0.3.0

nolusd config keyring-backend os
nolusd config chain-id nolus-rila
nolusd init "<Your moniker>" --chain-id nolus-rila
...
CONFIG_TOML="~/.nolusd/config/config.toml"
...
# Snapshot
URL="https://snapshots-testnet.state-town.com/nolus/nolus-rila_latest.tar.lz4"
curl -L $URL | lz4 -dc - | tar -xf - -C $HOME/.nolusd
```
