...
```bash
bash <(curl -s https://raw.githubusercontent.com/staketown/cosmos/master/andromeda/test_install.sh)
```
...
andromedad init "Your Moniker" --chain-id galileo-3
...
CONFIG_TOML="~/.andromedad/config/config.toml"
SEEDS=""
...
sed -i 's|^enable *=.*|enable = false|' $HOME/.andromedad/config/config.toml
