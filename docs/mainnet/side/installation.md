...
```bash
bash <(curl -s https://raw.githubusercontent.com/staketown/cosmos/master/side/main_install.sh)
```
...
sided init "Your Moniker" --chain-id sidechain-1
...
sed -i 's|^minimum-gas-prices *=.*|minimum-gas-prices = "0.0006uside,0.000001sat"|g' $CONFIG_TOML
