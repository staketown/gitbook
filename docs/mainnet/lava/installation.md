...
```bash
bash <(curl -s https://raw.githubusercontent.com/staketown/cosmos/master/lava/main_install.sh)
```
...
lavad init "Your Moniker" --chain-id lava-mainnet-1
...
sed -i 's|^minimum-gas-prices *=.*|minimum-gas-prices = "0.000000001ulava"|g' $CONFIG_TOML
