...
```bash
bash <(curl -s https://raw.githubusercontent.com/staketown/cosmos/master/juno/main_install.sh)
```
...
junod init "Your Moniker" --chain-id juno-1

# Download genesis and addrbook
curl -Ls https://snapshots.stake-town.com/juno/genesis.json > $HOME/.juno/config/genesis.json
curl -Ls https://snapshots.stake-town.com/juno/addrbook.json > $HOME/.juno/config/addrbook.json
...
SEEDS=""
PEERS="397281b9fa745482127093f08ac05724c2b8d6f8@65.108.195.213:33656"
sed -i.bak -e "s/^persistent_peers *=.*/persistent_peers = \"$PEERS\"/" $CONFIG_TOML
sed -i.bak -e "s/^seeds =.*/seeds = \"$SEEDS\"/" $CONFIG_TOML
external_address=$(wget -qO- eth0.me)
sed -i.bak -e "s/^external_address *=.*/external_address = \"$external_address:26656\"/" $CONFIG_TOML
sed -i 's|^minimum-gas-prices *=.*|minimum-gas-prices = "0ujuno"|g' $CONFIG_TOML
