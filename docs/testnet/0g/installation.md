...
```bash
bash <(curl -s https://raw.githubusercontent.com/staketown/cosmos/master/0g/test_install.sh)
```
...
```bash
snapshot_interval=0
sed -i.bak -e "s/^snapshot-interval *=.*/snapshot-interval = \"$snapshot_interval\"/" ~/.0gchain/config/app.toml
sed -i 's|^enable *=.*|enable = false|' $HOME/.0gchain/config/config.toml
